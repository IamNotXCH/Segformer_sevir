...
from dataloaders.sevir.sevir_dataloader import SEVIRDataLoader
...
class SEVIRTorchDataset(TorchDataset):
    ...
    def collate_fn(self, data_dict_list):
        ...
        # 假设你只使用 'vil' 通道
        vil_data = data_list_dict['vil']

        # 将原本的 (Batch, Height, Width, Time) 转换为 (Batch, Channels, Depth, Height, Width)
        # 例如，将 (N, H, W, T) -> (N, 1, T, H, W)
        vil_data = [torch.permute(data, (0, 3, 1, 2)).unsqueeze(1) for data in vil_data] # 从 (N, H, W, T) -> (N, 1, T, H, W)
        ...
        merged_data["vil"] = torch.cat(vil_data, dim=0)  # 确保 'vil' 数据正确堆叠
        
        merged_data["mask"] = None
        return merged_data
...
def check_aws():
    ...
    aws_cli_path = r"D:\aws\aws.exe"  # 手动指定 AWS CLI 的路径
    if os.system(f"{aws_cli_path} --version") != 0:
        raise RuntimeError("AWS CLI is not installed or cannot be found!")

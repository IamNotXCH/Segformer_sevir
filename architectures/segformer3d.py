...
class PatchEmbedding(nn.Module):
    def __init__(
        ...
        self.patch_embeddings = nn.Conv3d(
            in_channel,
            embed_dim,
            kernel_size=(3, 7, 7),  # 时间维度卷积核大小为3，空间维度为7x7
            stride=(1, 4, 4),       # 时间维度步长为1，空间维度为4
            padding=(1, 3, 3)       # 时间维度填充为1，空间维度为3
        )
...
class SegFormerDecoderHead(nn.Module):
    ...
    def __init__(
        ...
        self.linear_fuse = nn.Sequential(
            nn.Conv3d(
                in_channels=1 * decoder_head_embedding_dim,
        ...
        self.upsample_volume = nn.Upsample(
            scale_factor=(1.0, 4.0, 4.0), mode="trilinear", align_corners=False
        )
...
if __name__ == "__main__":
    input = torch.randint(
        low=0,
        high=255,
        size=(1, 1, 49, 384, 384),  # 1 表示 batch size, 1 表示单通道，49 表示时间维度，384x384 是空间维度
        dtype=torch.float,
    )
    input = input.to("cuda:0")
    segformer3D = SegFormer3D(in_channels=1).to("cuda:0")  # in_channels 设为 1 以适应单通道 VIL 数据
